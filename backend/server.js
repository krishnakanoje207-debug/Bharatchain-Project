...
// ===== START SERVER WITH AUTO-BOOTSTRAP =====
async function start() {
  ...
  // Auto-bootstrap: deploy contracts & sync DB state to fresh Hardhat chain
  try {
  ...
  // Detects when Hardhat node was restarted (all on-chain state lost)
  // and clears stale on_chain_citizen_id values so sync scheduler can re-register them
  try {
  ...
  console.log("   🔗 On-chain sync scheduler: every 120s\n");
    ...
    setInterval(async () => {
      // Use IST (UTC+5:30) since frontend sends scheduled times in IST
      const now = new Date();
